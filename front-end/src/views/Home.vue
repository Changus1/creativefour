<template>
<div class='page'>
  <div v-if="myBanner != null">
    <BannerList :banner="myBanner" />
  </div>
  <div v-else></div>
...
export default {
  ...
  methods: {
    ...
    async getAnimal() {
      try {
        let response = await axios.get("/api/animals/random");
        this.animal = response.data;
        this.animal = this.animal[0];
        return true;
      } catch (error) {
        console.log(error);
      }
    ...
    async getBanner() {
      try {
        let response = await axios.get("/api/banners/random");
        this.myBanner = response.data;
        this.myBanner = this.myBanner[0];
        return true;
      } catch (error) {
        console.log(error);
      }
